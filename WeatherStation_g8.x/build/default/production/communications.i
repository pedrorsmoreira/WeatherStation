...
void modify_pmon(char * data) {
    if(!data || strlen(data) != 1 || data[0] > 99) send_confirmation(0XC4, 0xFF);
    pmon = data[0];
    write_pmon(pmon);
...
void modify_time_alarm(char * data) {
    if(!data || strlen(data) != 1 || data[0] > 60) send_confirmation(0XC5, 0xFF);
    tala = data[0];
    write_tala(tala);
...
void define_alarm(char * data) {
    if(!data || strlen(data) != 2 || data[0] > 50 || data[1] > 3) send_confirmation(0XC7, 0xFF);
    alat = data[0]; alal = data[1];
    send_confirmation(0XC7, 0);
}

void switch_alarm(char * data) {
    if(!data || strlen(data) != 1 || data[0] > 1) send_confirmation(0XC8, 0xFF);
    alaf = data[0];
    write_alaf(alaf);
...
void registers_info() {
    uint8_t N = read_nreg(), wi = read_iwrt(), nr = ring_buffer_laped() ? N : wi, ri = 0;
...
void transfer_registers(char * data) {
    if(!data || strlen(data) != 1 || data[0] > read_nreg()) send_confirmation(0XCA, 0xFF);

}

void transfer_registers_i(char * data) {
    if(!data || strlen(data) != 2 || data[0] > read_nreg() || data[1] > read_nreg() - 1) send_confirmation(0XCB, 0xFF);
